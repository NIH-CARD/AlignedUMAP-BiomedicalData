...
def get_cylinder_plot(selected_data_dummy, input_color_column, color_mapping_original, height=1000, width=1300):
    ...
    def surface_ellipse_params(V, V_std):
        ...
        for i in range(0, len(z_surface)):
            ...
            x_temp = [x_surface[i] - x_surface_std[i], x_surface[i] - x_surface_std[i], x_surface[i] + x_surface_std[i],
                      x_surface[i] + x_surface_std[i]]
            y_temp = [y_surface[i] - y_surface_std[i], y_surface[i] + y_surface_std[i], y_surface[i] + y_surface_std[i],
                      y_surface[i] - y_surface_std[i]]
            # print (x_temp)
            # print (y_temp)
    ...
    def generate_plotly_background(selected_data, selected_genes, background_genes, new_color_mapping, new_line_mapping,
        ...
        if 'Median value1' in temp.columns:
            ...
            for e, dia in enumerate(list(temp['STUDY_DIAGNOSIS'].unique())):
                ...
                V_std = V.groupby('days from baseline').agg(get_95ci).dropna(
                    subset=['Median value1', 'Median value2']).sort_index()
                ...
                colorscale = [[0, c], [1, c]]
                x_new, y_new, z_new = surface_ellipse_params(V, V_std)
                # z, x, y
                # cyl1 = go.Surface(x=x_new, y=y_new, z=z_new, colorscale=colorscale, showscale=False, opacity=0.5, name=dia)
                line1 = go.Scatter3d(x=z, y=x, z=y, marker=dict( size=4, color=c), line=dict( color=c, width=4)) # colorscale='Viridis',
