...
from download_alignedumap import download_data
fname = f"results_data/{input_dataset_name}/{input_visualization_method}/generated_data/{input_dataset_name}_16.pickle"

# if os.path.exists(fname): # (len(os.listdir(dataset_p)) >= 1): os.path.exists(dataset_p) and
#    if st.sidebar.button("Clear data cache"):
#         os.system(f"rm -rf {dataset_p}")
#     info_msg2.info("Data already exists! No need to download.")
#     if cols[2].button("Remove Data"):
#         os.system(f"rm -rf {dataset_p}")
...
from apps import browsable_component, select, parameter_effect
